...
torch pins, so its dependency set is the one property the package has to keep. This checks what
the distribution metadata declares; the unit test workflow additionally resolves the built
wheel's full transitive tree and asserts that nothing heavy appears in it.
"""
...
def test_requires() -> None:
    requires = metadata.requires("lightly-studio-embed")
