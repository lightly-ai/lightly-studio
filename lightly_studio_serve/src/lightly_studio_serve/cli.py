"""Command line interface for LightlyStudio Serve.

``lightly-studio-serve conformance <url>`` ends with 0 for a server that passes, so a
customer can run it in their own build.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run one command and write its report.

    Args:
        argv: The arguments to read. ``None`` reads the ones of the process.

    Returns:
        The exit status: 0 for a command that passes, 1 for one that does not.
    """
    arguments = _parse(argv=argv)
    return _conformance(arguments=arguments)


def _parse(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog=PROGRAM, description="LightlyStudio Serve CLI.")
    commands = parser.add_subparsers(dest="command", required=True)
    conformance = commands.add_parser(
        "conformance",
        help="Check that a server implements the LightlyStudio embedding protocol.",
        description=(
    ...
    conformance.add_argument("url", help="The address of the server, e.g. http://127.0.0.1:8080.")
