<script lang="ts">
    ...
    const { removeTagFromSample } = $derived(useRemoveTagFromSample({ collectionId }));
    ...
    const labels = useAnnotationLabels({ collectionId: data.collection.collection_id });
    const { createLabel } = useCreateLabel({ collectionId: data.collection.collection_id });
...
<div class="flex h-full w-full flex-col space-y-4">
    <div class="flex w-full items-center">
        {#if $rootCollection.data}
            <FrameDetailsBreadcrumb rootCollection={$rootCollection.data} {frameIndex} />
        {/if}
    </div>
