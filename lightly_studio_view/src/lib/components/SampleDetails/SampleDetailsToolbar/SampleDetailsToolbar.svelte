<script lang="ts">
    ...
    const getBrushAnnotationType = () => {
        const isSemanticAnnotationInDetailsView =
            annotationLabelContext.isOnAnnotationDetailsView &&
            annotationLabelContext.annotationType === AnnotationType.SEMANTIC_SEGMENTATION;

        return isSemanticAnnotationInDetailsView
    ...
    $effect(() => {
        ...
        if (sampleDetailsToolbarContext.status === 'cursor') {
            ...
            if (sampleDetailsToolbarContext.status === 'bounding-box') {
                ...
                setAnnotationType(getBrushAnnotationType());
            }
        }
