<script lang="ts">
    ...
    import SamplingEstimatePreview from './SamplingEstimatePreview.svelte';
    import SamplingRunStatus from './SamplingRunStatus.svelte';
...
<Dialog.Root open={$isSamplingDialogOpen} onOpenChange={handleDialogOpenChange}>
    <Dialog.Portal>
        <Dialog.Overlay />
        <Dialog.Content class="border-border bg-background sm:max-w-[560px]">
            {#if $frozenRun}
                <Dialog.Header>
                    <Dialog.Title class="text-foreground">Sampling in progress</Dialog.Title>
                    <Dialog.Description class="text-foreground">
                        The selection is being created.
                    </Dialog.Description>
                </Dialog.Header>
                <SamplingRunStatus run={$frozenRun} loadingMessage={$loadingMessage} />
            {:else}
                <form onsubmit={handleFormSubmit}>
                    ...
                    <div class="max-h-[60vh] overflow-y-auto p-2 dark:[color-scheme:dark]">
                        <div class="grid gap-4 py-4">
                            ...
                            {#if $samplingEstimate}
                                <SamplingEstimatePreview estimate={$samplingEstimate} />
                            {/if}
                        </div>
                    </div>
