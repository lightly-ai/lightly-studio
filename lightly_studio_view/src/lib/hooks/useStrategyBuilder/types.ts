...
export interface MetadataClassBalancingParams {
    metadata_key: string;
    target_distribution_mode: ClassBalancingTargetDistributionMode;
    target_distribution: ClassBalancingTargetRow[];
    strength: number;
...
export interface StrategyParamsByType {
    ...
    metadata_class_balancing: MetadataClassBalancingParams;
}
...
export const STRATEGY_OPTIONS: { type: StrategyType; label: string; description: string }[] = [
    ...
    },
    {
        ...
        description:
            'Selects samples to reach a target distribution over a categorical metadata field. Balances one key at a time; add several to balance multiple keys.'
    }
