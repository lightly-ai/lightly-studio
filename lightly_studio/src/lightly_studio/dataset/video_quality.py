...
# Dense short bursts catch handheld shake and strobing; sparse whole-video
# samples confuse scene changes with temporal quality problems.
...
STROBE_SCORE_KEY = "strobe_score"
STROBE_BRIGHTNESS_CHANGE_MEAN_KEY = "strobe_brightness_change_mean"
...
class VideoQualityScores:
    ...
    strobe_score: float
    """Worst repeated brightness-reversal strength in ``[0, 1]``."""

    strobe_brightness_change_mean: float
    """Mean brightness change in the burst with the worst strobe score."""


@dataclass(frozen=True)
class StrobeMetrics:
    """Strobe measurements for one frame sequence."""

    score: float
    """Repeated brightness-reversal strength in ``[0, 1]``."""

    brightness_change_mean: float
    """Mean absolute brightness change in ``[0, 1]``."""
...
class VideoQualityAccumulator:
    """Collect sparse quality frames and dense temporal bursts from a decode.
    ...
    or inside a temporal burst, so the cost matches the seek-based scorer without the
    ...
    def finalize(self) -> VideoQualityScores | None:
        ...
        if not self._quality_frames:
            return None
        strobe = strobe_metrics_from_bursts(self._bursts)
        return _aggregate_frame_quality_with_temporal_scores(
...
def score_video_quality(
    ...
    *,
    ...
    """Score blur, lighting, motion, camera shake, and strobing for one video.

    Opens ``video_path`` once. Blur/lighting/motion use ``num_frames`` uniformly
    spaced samples. Shake and strobing use short dense bursts so high-frequency
    changes are not confused with scene changes.
...
def score_opened_video_quality(
    ...
    *,
    ...
    strobe = strobe_metrics_from_bursts(bursts)
    return _aggregate_frame_quality_with_temporal_scores(
...
def estimate_global_translation(
    ...
    if points is not None and len(points) >= _MIN_TRACKED_POINTS:
        ...
        if next_points is not None and status is not None:
            # The OpenCV stubs type these as ``Mat | ndarray``; both are ndarrays at runtime.
            ok = np.asarray(status).reshape(-1) == 1
            if int(np.count_nonzero(ok)) >= _MIN_TRACKED_POINTS:
                tracked = np.asarray(next_points)[ok].reshape(-1, 2)
                deltas = tracked - np.asarray(points)[ok].reshape(-1, 2)
...
def strobe_metrics(frames: Sequence[NDArray[np.uint8]]) -> StrobeMetrics:
    """Measure brightness change and repeated reversals in adjacent frames."""
    if len(frames) < _MIN_FRAMES_FOR_STROBE:
        return StrobeMetrics(score=0.0, brightness_change_mean=0.0)

    brightness = np.asarray([float(np.mean(frame)) for frame in frames], dtype=np.float32)
    changes = np.diff(brightness)
    absolute_changes = np.abs(changes)
    reversing = changes[:-1] * changes[1:] < 0
    reversal_strengths = np.where(
        reversing,
        np.minimum(absolute_changes[:-1], absolute_changes[1:]),
        0.0,
    )
    return StrobeMetrics(
        score=float(np.mean(reversal_strengths) / 255.0),
        brightness_change_mean=float(np.mean(absolute_changes) / 255.0),
    )


def strobe_metrics_from_bursts(
    bursts: Sequence[Sequence[NDArray[np.uint8]]],
) -> StrobeMetrics:
    """Return metrics from the burst with the strongest strobe signal."""
    metrics = [strobe_metrics(burst) for burst in bursts if len(burst) >= _MIN_FRAMES_FOR_STROBE]
    if not metrics:
        return StrobeMetrics(score=0.0, brightness_change_mean=0.0)
    return max(metrics, key=lambda metric: (metric.score, metric.brightness_change_mean))
...
def scores_to_metadata(scores: VideoQualityScores) -> dict[str, float]:
    ...
    return {
        ...
        STROBE_SCORE_KEY: scores.strobe_score,
        STROBE_BRIGHTNESS_CHANGE_MEAN_KEY: scores.strobe_brightness_change_mean,
    }
...
def _aggregate_frame_quality_with_temporal_scores(
    frames: Sequence[NDArray[np.uint8]],
    shake_score: float,
    strobe: StrobeMetrics,
) -> VideoQualityScores:
    """Aggregate per-frame metrics and pre-computed temporal scores.

    Shake and strobing need dense frames, so they are scored separately from the
    sparse frames used for blur, lighting, and motion.

    Args:
        frames: Grayscale ``uint8`` frames. Must be non-empty.
        shake_score: Pre-computed camera shake score.
        strobe: Pre-computed strobe measurements.
    ...
    return VideoQualityScores(
        ...
        strobe_score=strobe.score,
        strobe_brightness_change_mean=strobe.brightness_change_mean,
    )
...
__all__ = [
    ...
    "DEFAULT_STROBE_SCORE_HIGH_MIN",
    "LIGHTING_SCORE_KEY",
    "MOTION_SCORE_KEY",
    "OVEREXPOSURE_RATIO_KEY",
    "SHAKE_SCORE_KEY",
    "STROBE_BRIGHTNESS_CHANGE_MEAN_KEY",
    "STROBE_SCORE_KEY",
