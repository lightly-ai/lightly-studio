...
def from_decoded_message(decoded_message: Any) -> int:
    ...
    Raises:
        McapAccessError: If the message has no `header.stamp` or `timestamp`.
    """
...
def _timestamp_ns(stamp: Any) -> int:
    ...
    sec = int(message_fields.require_field(stamp, _SEC_FIELDS))
    nsec = message_fields.get_field(stamp, _NSEC_FIELDS)
    return sec * NANOSECONDS_PER_SECOND + (0 if nsec is None else int(nsec))
