...
def read_point_cloud_field(
    data: bytes, field: object, layout: PointCloudLayout
) -> NDArray[Any]:
    """Read a scalar PointField as a flat numeric array."""
    datatype = int(point_cloud_value.get_value(field, "datatype"))
    if (
        datatype not in POINT_FIELD_DTYPES
        or int(point_cloud_value.get_value(field, "count")) != 1
    ):
        raise McapAccessError(
            f"Unsupported PointCloud2 field '{point_cloud_value.get_value(field, 'name')}'."
        )
