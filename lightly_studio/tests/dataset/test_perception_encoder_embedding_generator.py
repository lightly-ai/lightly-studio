...
class TestPerceptionEncoderEmbeddingGenerator:
    ...
    def test_embed_videos(self) -> None:
        ...
        assert np.isclose(dog_video_embedding_normed[0], 0.0287, atol=1e-4)
        assert np.isclose(dog_video_embedding_normed[1], 0.0574, atol=1e-4)
        assert np.isclose(dog_video_embedding_normed[2], 0.0567, atol=1e-4)
        assert np.isclose(dog_video_embedding_normed[3], -0.0770, atol=1e-4)
    ...
    def test_classification_video(self) -> None:
        ...
        assert np.isclose(text_probs[0], 0.72, atol=1e-2)
        assert np.isclose(text_probs[1], 0.14, atol=1e-2)
        assert np.isclose(text_probs[2], 0.14, atol=1e-2)
