...
def test_compute_typicality_metadata(db_session: Session) -> None:
    ...
    compute_typicality.compute_typicality_metadata(
        session=db_session, collection_id=collection_id, embedding_model_id=embedding_model_id
    )

    samples = list(DatasetQuery(collection, db_session))
