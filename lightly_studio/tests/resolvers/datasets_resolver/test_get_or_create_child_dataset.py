...
def test_get_or_create_child_dataset(
    ...
    video_frames_dataset_id = dataset_resolver.get_or_create_child_dataset(
        session=db_session, dataset_id=video_dataset.dataset_id, sample_type=SampleType.VIDEO_FRAME
    )
    ...
    same_video_frames_dataset_id = dataset_resolver.get_or_create_child_dataset(
        session=db_session, dataset_id=video_dataset.dataset_id, sample_type=SampleType.VIDEO_FRAME
    )
...
def test_get_or_create_child_dataset__non_existent(
    ...
    with pytest.raises(ValueError, match=f"Dataset with id {non_existent_id} not found"):
        dataset_resolver.get_or_create_child_dataset(
            session=db_session,
...
def test_get_or_create_child_dataset__return_direct_child(
    ...
    fetched_child_dataset_id = dataset_resolver.get_or_create_child_dataset(
        session=db_session, dataset_id=dataset_id, sample_type=SampleType.CAPTION
    )
