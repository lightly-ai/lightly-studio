...
def test_get_dimension_bounds(
    ...
    bounds = image_resolver.get_dimension_bounds(session=test_db, collection_id=dataset_id)
...
def test_get_dimension_bounds__with_tag_filtering(
    ...
    bounds = image_resolver.get_dimension_bounds(
        session=test_db, collection_id=dataset_id, tag_ids=[tag_bigger.tag_id]
    )
    ...
    bounds = image_resolver.get_dimension_bounds(
        session=test_db, collection_id=dataset_id, tag_ids=[tag_smaller.tag_id]
    )
    assert bounds["min_width"] == 100
