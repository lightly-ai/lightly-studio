...
def test_build_sample_tag_counts__preserves_order_and_zero_fills() -> None:
    ...
    assert result[0].counts == [AnnotationClassCount("cat", 1), AnnotationClassCount("dog", 0)]
    assert result[1].sample_tag_name == "tag-a"
    assert result[1].counts == [AnnotationClassCount("cat", 0), AnnotationClassCount("dog", 3)]
