...
def test_create_many_and_get_many_by_id(db_session: Session) -> None:
    ...
    creates = [
        SampleCreate(collection_id=dataset1_id),
    ...
    assert fetched_samples[0].collection_id == dataset1_id
    assert fetched_samples[1].sample_id == sample_ids[1]
    assert fetched_samples[1].collection_id == dataset2_id
    assert fetched_samples[2].sample_id == sample_ids[2]
    assert fetched_samples[2].collection_id == dataset1_id
