...
def test_get_sample_type_by_id(db_session: Session) -> None:
    ...
    sample_type = sample_resolver.get_sample_type(session=db_session, sample_id=sample.sample_id)
    assert sample_type == SampleType.IMAGE


def test_get_sample_type_video(db_session: Session) -> None:
...
def test_get_sample_type_group(db_session: Session) -> None:
...
def test_get_sample_type_nonexistent(db_session: Session) -> None:
    """Test getting sample type for non-existent sample raises ValueError."""
    import uuid

    non_existent_id = uuid.uuid4()
    with pytest.raises(ValueError, match="does not exist"):
        sample_resolver.get_sample_type(session=db_session, sample_id=non_existent_id)
