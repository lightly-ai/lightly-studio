...
class TestImageFilter:
    ...
    def test_apply__query_expr_filter(self, db_session: Session) -> None:
        ...
        samples = create_images(
            ...
            collection_id=collection.collection_id,
            images=[
                ImageStub(path="small.png", width=100),
                ImageStub(path="large.png", width=800),
            ],
        )
        ...
        query = select(ImageTable).join(ImageTable.sample)
        result = db_session.exec(image_filter.apply(query=query)).all()

        assert len(result) == 1
        assert result[0].sample_id == samples[1].sample_id

    def test_apply__query_expr_with_and(self, db_session: Session) -> None:
        collection = create_collection(session=db_session)
        samples = create_images(
            db_session=db_session,
            collection_id=collection.collection_id,
            images=[
                ImageStub(path="a.png", width=800, height=600),
                ImageStub(path="b.png", width=800, height=200),
                ImageStub(path="c.png", width=200, height=200),
            ],
        )
    ...
    def test_apply__query_expr_combined_with_sample_ids(self, db_session: Session) -> None:
        collection = create_collection(session=db_session)
        samples = create_images(
            db_session=db_session,
            collection_id=collection.collection_id,
