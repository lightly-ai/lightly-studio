...
class TestDatasetQueryAddTag:
    def test_add_tag__with_expressions(self, db_session: Session) -> None:
        ...
        tag = tag_resolver.get_by_name(
            session=db_session, tag_name="my_tag", collection_id=dataset.collection_id
        )
        ...
        db_session.refresh(image20)
        db_session.refresh(image30)
        db_session.refresh(image40)
    ...
    def test_add_tag__no_expressions(self, db_session: Session) -> None:
    ...
    def test_add_tag__double_tag(self, db_session: Session) -> None:
    ...
    def test_add_tag__empty_query(self, db_session: Session) -> None:
        ...
        tag = tag_resolver.get_by_name(
            session=db_session, tag_name="example_tag", collection_id=dataset.collection_id
        )
        assert tag is not None
    ...
    def test_add_tag__some_already_tagged(self, db_session: Session) -> None:
        ...
        tag_resolver.add_tag_to_sample(session=db_session, tag_id=tag.tag_id, sample=image2.sample)
