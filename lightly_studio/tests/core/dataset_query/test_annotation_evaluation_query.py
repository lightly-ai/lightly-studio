...
def test_annotation_metric_query__matches_false_positive_criteria_on_same_prediction(
    db_session: Session,
) -> None:
    collection = create_collection(session=db_session)
    [image_a, image_b] = create_images(
        db_session=db_session,
        collection_id=collection.collection_id,
        images=[ImageStub(path="fp-dog-low.jpg"), ImageStub(path="fp-dog-high.jpg")],
    )
    run = create_run(session=db_session, collection_id=collection.collection_id, name="run1")
    dog_label = create_annotation_label(
        session=db_session,
    ...
    results = DatasetQuery(dataset=collection, session=db_session).match(
        AnnotationMetricQuery.false_positive(
            "run1",
            "dog",
            AnnotationEvaluationMetricField("confidence") > 0.5,
        )
    )
