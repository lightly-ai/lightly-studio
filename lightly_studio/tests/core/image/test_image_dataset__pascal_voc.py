...
class TestImageDataset:
    def test_add_samples_from_pascal_voc_segmentations(  # noqa: PLR0915
        ...
        Image.fromarray(mask1).save(masks_path / "image1.png")
        mask2 = np.array(
            [
                [0, 0, 0],
                [0, 0, 0],
        ...
        Image.fromarray(mask2).save(masks_path / "image2.png")
    ...
    def test_add_samples_from_pascal_voc_segmentations__preserve_remote_paths(  # noqa: PLR0915
        self,
        patch_collection: None,  # noqa: ARG002
    ) -> None:
        ...
        with fsspec.open(f"{images_path}/image1.jpg", "wb") as image_file:
            Image.new("RGB", (4, 3)).save(image_file, format="JPEG")
        with fsspec.open(f"{images_path}/image2.jpg", "wb") as image_file:
        ...
        with fsspec.open(f"{masks_path}/image1.png", "wb") as mask_file:
        ...
        with fsspec.open(f"{masks_path}/image2.png", "wb") as mask_file:
        ...
        assert len(samples) == 2
        assert [s.file_name for s in samples] == ["image1.jpg", "image2.jpg"]
        assert samples[0].file_path_abs == f"{images_path}/image1.jpg"
        assert samples[1].file_path_abs == f"{images_path}/image2.jpg"
        assert all(
            len(s.sample_table.embeddings) == 1 for s in samples
        )  # Embeddings should be generated
