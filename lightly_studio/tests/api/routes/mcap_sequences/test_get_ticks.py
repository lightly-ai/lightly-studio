...
def test_get_ticks__unknown_sequence(test_client: TestClient) -> None:
    response = test_client.get(f"/datasets/{uuid.uuid4()}/mcap-sequences/{uuid.uuid4()}/ticks")
...
def test_get_ticks__wrong_dataset(test_client: TestClient, db_session: Session) -> None:
    ...
    response = test_client.get(f"/datasets/{uuid.uuid4()}/mcap-sequences/{fixture.sample_id}/ticks")
...
def _add_ticks(session: Session, fixture: McapSequenceFixture) -> None:
    ...
    for seq_number, (log_time_ns, keyframe_log_time_ns) in enumerate([(_T0, _T0), (_T1, _T1)]):
