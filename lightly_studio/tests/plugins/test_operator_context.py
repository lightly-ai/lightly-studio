...
def test_get_allowed_scopes_for_collection(sample_type: SampleType) -> None:
    ...
    assert get_allowed_scopes_for_collection(sample_type=sample_type, is_root_collection=False) == [
        scope
    ]
    assert get_allowed_scopes_for_collection(sample_type=sample_type, is_root_collection=True) == [
        OperatorScope.ROOT,
        scope,
    ]
