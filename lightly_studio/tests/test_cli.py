...
def test_gui(mocker: MockerFixture) -> None:
    ...
    assert result.exit_code == 0
    mock_connect.assert_called_once_with(db_file=None, engine_url=None)
...
def test_gui__with_db_file(mocker: MockerFixture) -> None:
    ...
    mock_start_gui = mocker.patch.object(lightly_studio, "start_gui")
    runner = CliRunner()
    result = runner.invoke(cli=cli.main, args=["gui", "--db-file", "my.duckdb"])
    assert result.exit_code == 0
    mock_connect.assert_called_once_with(db_file="my.duckdb", engine_url=None)
    mock_start_gui.assert_called_once_with(host=None, port=None)
...
def test_gui__with_db_url(mocker: MockerFixture) -> None:
    ...
    assert result.exit_code == 0
    mock_connect.assert_called_once_with(db_file=None, engine_url="postgresql://localhost/mydb")
    mock_start_gui.assert_called_once_with(host=None, port=None)
