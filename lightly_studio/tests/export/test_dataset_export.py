"""Tests for the sample-type-agnostic export format logic in `DatasetExport`.

`DatasetExport` holds all the format logic; its subclasses only bind a `sample_to_image`
strategy. These tests exercise that shared logic with the image mapping as the substrate.
The image mapping itself is tested in `test_image_dataset_export.py`.
"""
...
from lightly_studio.export import dataset_export, image_dataset_export
...
def _dataset_export(
    *,
    ...
    """Builds a `DatasetExport` using the image sample-to-image mapping as the test substrate."""
