...
def test_in_array__postgresql() -> None:
    ...
    expr = in_array(column=col(SampleTable.sample_id), values=[uuid.uuid4(), uuid.uuid4()])
...
def test_in_array__unsupported() -> None:
    expr = in_array(column=col(SampleTable.sample_id), values=[uuid.uuid4()])
...
def test_in_array__returns_matching_rows(db_session: Session) -> None:
    ...
    result = db_session.exec(
        select(SampleTable).where(in_array(column=col(SampleTable.sample_id), values=wanted))
    ).all()
...
def test_in_array__exceeds_postgres_param_limit(db_session: Session) -> None:
    ...
    result = db_session.exec(
        select(SampleTable).where(in_array(column=col(SampleTable.sample_id), values=sample_ids))
    ).all()
    assert result == []
