...
2. **Edit annotations directly in the annotation view.** For object detection, switch to the annotation view, which shows each annotation as a cropped-out image. This makes it easy to skim through many objects at once. Combine it with the class filter in the left panel to look at one class at a time — a grid of objects from a single class makes outliers, such as wrong classes or bad bounding boxes, much easier to spot. This approach is recommended if you are doing the QA yourself. See [Find Wrong Annotations](../workflows/annotation_qa.md) for two further ways to find wrong annotations.
