...
You can run the web UI in the background and continue executing Python code from your main script.
This is especially useful in notebooks where you want to keep exploring or updating the dataset
after the GUI starts. You can explore the dataset interactively in the GUI, then decide to add
some metadata or do selections in Python, and then continue exploring the updated dataset in the
GUI without having to restart it.
...
- Call `start_gui_background()` before interacting with the GUI and `stop_gui_background()` when
  you are done.
