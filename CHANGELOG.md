...
- Python SDK (beta): Custom embedding generators now implement `embedding_space_spec`, returning the new `EmbeddingSpaceSpec` (`space_key`, `dimension`) instead of `get_embedding_model_input`, which returned `EmbeddingSpaceDescription`.
